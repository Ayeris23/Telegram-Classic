...
- (void)sendVoiceMessage
{
    ...
    // 1. Extract PCM from CAF (KEEP THIS for now)
    FILE *pcm = caf_extract(cafFile.UTF8String, NULL);
    if (pcm == NULL)
    {
        dispatch_sync(dispatch_get_main_queue(), ^{
            [self.appDelegate showMessage:@"can't extract data from caf file"];
    ...
    // 2. Read PCM into memory buffer
    fseek(pcm, 0, SEEK_END);
    long size = ftell(pcm);
    fseek(pcm, 0, SEEK_SET);
    
    uint8_t *buffer = malloc(size);
    fread(buffer, 1, size, pcm);
    fclose(pcm);
    ...
    NSFileHandle *file =
    [NSFileHandle fileHandleForWritingAtPath:oggFile];
    
    if (!file)
    {
        file = [NSFileHandle fileHandleForWritingAtPath:oggFile];
        [@"" writeToFile:oggFile atomically:YES encoding:NSUTF8StringEncoding error:nil];
        file = [NSFileHandle fileHandleForWritingAtPath:oggFile];
    }
    
    TGOggOpusWriter *writer = [[TGOggOpusWriter alloc] init];
    
    if (![writer begin:file])
    {
        dispatch_sync(dispatch_get_main_queue(), ^{
            [self.appDelegate showMessage:@"opus encoder init failed"];
        });
        free(buffer);
        return;
    }
    
    // 4. Encode full buffer as single stream
    NSUInteger frameSize = 960 * 2; // 16-bit mono assumption
    
    for (NSUInteger offset = 0; offset < size; offset += frameSize)
    {
        NSUInteger chunkSize = MIN(frameSize, size - offset);
        
        [writer writeFrame:(uint8_t *)(buffer + offset)
            frameByteCount:chunkSize];
    }
    ...
    // 5. Send result
    tg_document_t *vm =
    tg_voice_message(self.appDelegate.tg, oggFile.UTF8String);
