...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    self.smallPhotoCache = [cache stringByAppendingPathComponent:@"s"];
    self.peerPhotoCache = [cache stringByAppendingPathComponent:@"peer"];
    self.imagesCache = [cache stringByAppendingPathComponent:@"images"];
    self.filesCache = [cache stringByAppendingPathComponent:@"files"];
    self.thumbDocCache = [cache stringByAppendingPathComponent:@"docThumbs"];
    
    NSArray *cacheDirectories = [NSArray arrayWithObjects:
                                 self.smallPhotoCache,
                                 self.peerPhotoCache,
                                 self.imagesCache,
                                 self.filesCache,
                                 self.thumbDocCache,
    ...
    for (NSString *directory in cacheDirectories) {
        ...
        if (![fileManager createDirectoryAtPath:directory
                    withIntermediateDirectories:YES
                                     attributes:nil
                                          error:&error]) {
            NSLog(@"Failed to create cache directory %@: %@", directory, error);
        }
    }
	...
	// Reachability retains these blocks, while AppDelegate retains Reachability.
	// Capture AppDelegate weakly to avoid a retain cycle.
	...
	self.reach.reachableBlock = ^(Reachability *reach)
	{
		...
		dispatch_async(dispatch_get_main_queue(), ^{
			if (strongSelf.reachabilityDelegate)
				[strongSelf.reachabilityDelegate isOnLine];
			NSLog(@"REACHABLE!");
		});
	};
	...
	self.reach.unreachableBlock = ^(Reachability *reach)
	{
		...
		dispatch_async(dispatch_get_main_queue(), ^{
			if (strongSelf.reachabilityDelegate)
				[strongSelf.reachabilityDelegate isOffLine];
		});
...
// Declared in AppDelegate.h; this wrapper completes the implementation.
...
-(void)application:(UIApplication *)application didReceiveRemoteNotification:(NSDictionary *)userInfo{
	...
	if(application.applicationState == UIApplicationStateInactive)
	{
        ...
        if (nc){
            ...
            [NSString stringWithFormat:@"%ld", (unsigned long)self.unread.count];
        }
...
- (void)loadTgLib
{
    ...
    int apiId = TELEGRAM_API_ID;
    const char *apiHash = TELEGRAM_API_HASH;
    
    NSLog(@"AppDelegate API ID: %d", apiId);
    NSLog(@"AppDelegate API hash length: %lu",
          apiHash ? (unsigned long)strlen(apiHash) : 0UL);
...
- (void)sendCode:(NSString *)phone_number
{
    ...
    NSLog(@"sendCode: requesting code for %@", phone_number);
    
    tl_auth_sentCode_t *sentCode =
    tg_auth_sendCode(self.tg, [phone_number UTF8String]);
    
    NSLog(@"sendCode: tg_auth_sendCode returned %p", sentCode);
    
    if (sentCode == NULL) {
        UIAlertView *alert =
        [[UIAlertView alloc] initWithTitle:@"Login failed"
                                   message:@"Telegram did not return a verification-code response. Check the Xcode console for the server error."
                                  delegate:nil
                         cancelButtonTitle:@"OK"
                         otherButtonTitles:nil];
